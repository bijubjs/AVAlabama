...
```{r}
pf_solar_r25 <- solar_profit %>% 
  filter(sprop == 1,
         elcprc == 0.04) %>%
  select(al_regs, array, height, eannprof, eannprofworeap)
cat("Maximum profit from solar at 100% PVD at 25% REAP = ",
    max(pf_solar_r25$eannprof),
    fill = TRUE)
pf_solar_r25[which.max(pf_solar_r25$eannprof),]
cat("Minimum profit from solar at 100% PVD at 25% REAP = ",
    min(pf_solar_r25$eannprof),
    fill = TRUE)
pf_solar_r25[which.min(pf_solar_r25$eannprof),]
```
