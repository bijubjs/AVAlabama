...
```{r, error=TRUE, warning=TRUE, echo=TRUE, eval=TRUE}
...
-   Profit from solar alone at their respective price is subtracted from tav_profit.

-   tavp_wosp = tav_profit - profit from solar at their respective prices. This variable gives an idea where av profit stands in relation to solar alone profit. It helps to identify relative profitability of agrivoltaic system compared to solar only.
...
## Calculate sbavp_wosp

-   Profit from solar alone at their respective price is subtracted from sbav_profit.

-   sbavp_wosp = sbav_profit - profit from solar at their respective prices. This variable gives an idea where av profit stands in relation to solar alone profit. It helps to identify relative profitability of agrivoltaic system compared to solar only.
...
```{r, eval=TRUE}
write.csv(as.data.frame(sbav_be_pden),
          row.names = TRUE,
          #col.names = TRUE,
          file = "Results/sbav_brkevn_pden.csv")
dim(sbav_be_pden)
```
...
## Calculate sqavp_wosp

-   Profit from solar alone at their respective price is subtracted from sqav_profit.

-   sqavp_wosp = sqav_profit - profit from solar at their respective prices. This variable gives an idea where av profit stands in relation to solar alone profit. It helps to identify relative profitability of agrivoltaic system compared to solar only.

```{r, echo=TRUE, eval=TRUE,results='hide', warning=TRUE, error=TRUE}
...
```{r,echo=TRUE,eval=FALSE,warning=TRUE, error=TRUE}
...
```{r, eval=TRUE, results='hide', warning=TRUE,error=TRUE, echo=TRUE}
...
```{r, eval=TRUE,results='hide',echo=TRUE,error=TRUE,warning=TRUE}
...
height <- c(4.6, 6.4, 8.2) # Panel height
al_regs <- c("Northern", "Central", "Black Belt", "Southern")
price <- c(17, 19, 21, 23) # Crop Price
elcprc <- c(0.02, 0.03, 0.04) # Electricity Price
yldvar <- c(0.40, 0.50, 0.60, 0.70, 0.80)

# Define the required columns
required_columns <- c("yldvar", "array", "height", 
                      "al_regs", "price", "elcprc")
...
missing_columns <- setdiff(required_columns, 
                           names(tav_be_pden))
if (length(missing_columns) > 0) {
  stop("Missing columns in tavp_be_pden: ",
       paste(missing_columns, collapse = ", "))
}

# Generate column names using reversed order of expand.grid
col_names <- apply(expand.grid(height, array, yldvar), 1,
                   function(x) paste0(x[3] , x[2] , x[1]))
...
# reversed column and row names
merged_data$col_name <- apply(
  merged_data[, c("yldvar", "array", "height")], 1,
  function(x) paste0(x[1], x[2], x[3]))
...
for (i in seq_len(nrow(result_matrix))) {
  ...
  if (nrow(row_data) > 0) {
    result_matrix[i, 
                  match(row_data$col_name, 
                        colnames(result_matrix))] <- row_data$sprop
  }
}
ct_tav_be_pden <- as.data.frame(result_matrix) # Table in Excel.
write.csv(as.data.frame(ct_tav_be_pden),
          row.names = TRUE,
          file = "Results/ct_tav_be_pden.csv")
dim(ct_tav_be_pden)
rm(result_matrix)
```
...
```{r}
uniquevalue = unique(as.vector(as.matrix(ct_tav_be_pden[-1])))
uniquevalue
colorcount = length(unique(as.vector(as.matrix(ct_tav_be_pden[-1]))))
colorcount
heatmap_plot <- pheatmap(t(ct_tav_be_pden),
                         #clustering_distance_rows = "correlation",
                         clustering_distance_rows = "euclidean",
                         clustering_distance_cols = "euclidean",
                         clustering_method = "complete",
                         ...
                         display_numbers = TRUE,
                         number_format = "%.2f",
                         legend_breaks = c(0.00, 0.25, 0.50, 0.75, 1.00)
                         )
```
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/gp_tav_be_pden", ".png"))
rm(heatmap_plot); rm(colorcount); rm(uniquevalue)
```
...
```{r, eval=FALSE, results='hide', warning=TRUE, error=TRUE, cache=TRUE}
...
```{r, eval=FALSE, warning=TRUE, results='hide', error=TRUE, cache=TRUE}
combinations <- expand.grid(
...
```{r, eval=FALSE,warning=FALSE,error=TRUE,results='hide',echo=TRUE}
...
height <- c(4.6, 6.4, 8.2) # Panel height
al_regs <- c("Northern", "Central", "Black Belt", "Southern")
price <- c(3, 6, 9) # Crop Price
elcprc <- c(0.02, 0.03, 0.04) # Electricity Price
yldvar <- c(0.50, 0.60, 0.90, 1)
...
missing_columns <- setdiff(required_columns, 
                           names(sbav_be_pden))
if (length(missing_columns) > 0) {
  stop("Missing columns in sbavp_be_pden: ",
       paste(missing_columns, collapse = ", "))
}
...
ct_sbav_be_pden <- as.data.frame(result_matrix) # Table in Excel.
write.csv(as.data.frame(ct_sbav_be_pden),
          row.names = TRUE,
          file = "Results/ct_sbav_be_pden.csv")
dim(ct_sbav_be_pden)
rm(result_matrix)
```

## sbav_be_pden Heatmap

```{r}
uniquevalue = unique(as.vector(as.matrix(ct_sbav_be_pden[-1])))
uniquevalue
colorcount = length(unique(as.vector(as.matrix(ct_sbav_be_pden[-1]))))
colorcount
heatmap_plot <- pheatmap(t(ct_sbav_be_pden),
                         #clustering_distance_rows = "correlation",
                         clustering_distance_rows = "euclidean",
                         clustering_distance_cols = "euclidean",
                         clustering_method = "complete",
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/gp_sbav_be_pden", ".png"))
rm(heatmap_plot); rm(colorcount); rm(uniquevalue)
```
...
```{r, echo=TRUE,warning=TRUE,results='hide',error=TRUE,eval=FALSE,cache=TRUE}
combinations <- expand.grid(
  yldvar = c(0, 0.1, 0.3, 0.5, 0.7, 1, 1.20, 1.5, 1.80, 2), # Yield
...
```{r, eval=FALSE,results='hide',echo=TRUE,warning=TRUE,error=TRUE,cache=TRUE}
combinations <- expand.grid(
  sprop = c(0, 0.25, 0.50, 0.75, 1.00), # Solar proportion
  price = c(3, 6, 9), # Strawberry price
  elcprc = c(0.03, 0.04, 0.05) #Electricity price
...
```{r, echo=TRUE, eval=FALSE,results='hide', warning=FALSE, error=FALSE}
# Define the values for each variable
sprop <- c(0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50,
           0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00) # Land Proportion
array <- c("Fixed", "Tracking")
...
```{r, echo=TRUE, eval=TRUE,results='hide', warning=FALSE, error=FALSE}
#Shape crosstab.
# Define the values for each variable
sprop <- c(0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50,
           0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00) # Land Proportion
...
```{r, echo=TRUE, eval=TRUE,results='hide', warning=FALSE, error=FALSE}
# Define the values for each variable
sprop <- c(0.05, 0.10, 0.20, 0.30, 0.40, 0.45, 0.50)
array <- c("Fixed", "Tracking") # Solar Array
height <- c(4.6, 6.4, 8.2) # Panel height
al_regs <- c("Northern", "Central", "Black Belt", "Southern")
price <- c(11, 14, 17)
...
height <- c(4.6, 6.4, 8.2) # Panel height
al_regs <- c("Northern", "Central", "Black Belt", "Southern")
price <- c(11, 14, 17) # Crop Price
elcprc <- c(0.02, 0.03, 0.04) # Electricity Price
yldvar <- c(0.20, 0.30, 0.40, 0.60, 0.70)
...
ct_sqav_be_pden <- as.data.frame(result_matrix) # Table in Excel.
write.csv(as.data.frame(ct_sqav_be_pden),
          row.names = TRUE,
          file = "Results/ct_sqav_be_pden.csv")
dim(ct_sqav_be_pden)
rm(result_matrix)
```

## sqav_be_pden Heatmap

```{r, echo=TRUE, eval=TRUE, results='hide', warning=FALSE, error=FALSE}
uniquevalue = unique(as.vector(as.matrix(ct_sqav_be_pden[-1])))
uniquevalue
colorcount = length(unique(as.vector(as.matrix(ct_sqav_be_pden[-1]))))
colorcount
heatmap_plot <- pheatmap(t(ct_sqav_be_pden),
                         #clustering_distance_rows = "correlation",
                         clustering_distance_rows = "euclidean",
                         clustering_distance_cols = "euclidean",
                         clustering_method = "complete",
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/gp_sqav_be_pden", ".png"))
rm(heatmap_plot); rm(colorcount); rm(uniquevalue)
```
