...
write_xlsx(x = tav_be_yld,
           file = "Results/TAV Tomato Breakeven Yield.xlsx",
           as_table = TRUE)
```
...
write_xlsx(x = sbav_be_yld,
           file = "Results/SBAV Strawberry Breakeven Yield.xlsx",
           as.table = TRUE)
dim(sbav_be_yld)
```
...
write_xlsx(x = ct_tav_pft %>%
  ...
  dplyr::select(Row_Names, everything()),
           file = "Results/Profit Crosstabed TAV.xlsx",
           as_table = TRUE)
dim(ct_tav_pft)
```
...
ggsave(heatmap_plot,
       ...
       units = "in",
       limitsize = FALSE,
       file = paste0("Plots/TAV Profits Crosstabbed", ".png"))
...
write_xlsx(x = ct_sbav_pft %>%
  ...
  dplyr::select(Row_Names, everything()),
           file = "Results/Profit Crosstabed SBAV.xlsx",
           as_table = TRUE)
dim(ct_sbav_pft)
```
...
colorcount
heatmap_plot <- pheatmap((ct_sbav_pft),
                         #clustering_distance_rows = "correlation",
                         clustering_distance_rows = "euclidean",
                         clustering_distance_cols = "euclidean",
                         clustering_method = "complete",
...
ggsave(heatmap_plot,
       ...
       units = "in",
       limitsize = FALSE,
       file = paste0("Plots/SBAV Profits Crosstabbed", ".png"))
rm(heatmap_plot);
```
