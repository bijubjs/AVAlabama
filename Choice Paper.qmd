...
# Initialize the result data frame
#tomatoav_profit <- data.frame(matrix(nrow = 813120, ncol = 17))
stbryAV_profit <- data.frame(
  matrix(nrow = nrow(stberry_long)*nrow(solar_profit),
         ncol = 18))
...
# Loop through each value of the second matrix
for (i in 1:nrow(stberry_long)) {
  f() #Progress bar.
  # Loop through each value of the second matrix
  for (j in 1:nrow(solar_profit)) {
    # First matrix, second matrix, combined two matrices.
    new_row <- c(solar_profit[j, ], stberry_long[i, ], 
                 solar_profit[j, 13] + stberry_long[i, 4])
    # Assign the new row to the result matrix
    stbryAV_profit[row_index, ] <- new_row
    # Increment the row index
    row_index <- row_index + 1
  }
}
# Name the columns
colnames(stbryAV_profit) <- c(colnames(solar_profit), 
                               colnames(stberry_long),
                               "stav_prft")

# Display the result
dim(stbryAV_profit)
head(stbryAV_profit); tail(stbryAV_profit)
```
...
# Initialize the result data frame
#tomatoav_profit <- data.frame(matrix(nrow = 813120, ncol = 17))
squashAV_profit <- data.frame(
  matrix(nrow = nrow(squash_long)*nrow(solar_profit),
         ncol = 18))
...
# Loop through each value of the second matrix
for (i in 1:nrow(squash_long)) {
  f() #Progress bar.
  # Loop through each value of the second matrix
  for (j in 1:nrow(solar_profit)) {
    # First matrix, second matrix, combined two matrices.
    new_row <- c(solar_profit[j, ], squash_long[i, ], 
                 solar_profit[j, 13] + squash_long[i, 4])
    # Assign the new row to the result matrix
    squashAV_profit[row_index, ] <- new_row
    # Increment the row index
    row_index <- row_index + 1
  }
}
# Name the columns
colnames(squashAV_profit) <- c(colnames(solar_profit), 
                               colnames(squash_long),
                               "sqav_prft")

# Display the result
dim(squashAV_profit)
head(squashAV_profit); tail(squashAV_profit)
```
