...
```{r Libs,,echo=TRUE,eval=TRUE,error=TRUE,warning=TRUE}
...
```{r Theme, echo=FALSE, eval=TRUE, error=TRUE, warning=TRUE}
####### Plotting Data: #####
# Map Theme:
plottheme <- ggplot() +
  theme_void() +
...
```{r ImportSQ,echo=TRUE,eval=FALSE,results='hide',error=TRUE,warning=TRUE}
...
```{r EOBDCSS,echo=TRUE,eval=TRUE,error=TRUE,warning=TRUE}
lox <- c("Northern", "Central", "Black Belt", "Southern")
ggplot(data = energy_output,
         mapping = aes(x = al_regs,
                       y = energy,
...
```{r Sim2,echo=TRUE,eval=FALSE,results='hide',error=TRUE,warning=TRUE}
## | results='hide'
# Sample data
set.seed(123)
...
### Breakdown by proportion of land under solar
...
```{r BDBPLUS, eval=TRUE, echo=TRUE, warning=TRUE, error=TRUE}
lox <- c("Northern", "Central", "Black Belt", "Southern")
array_levs = c("Single Axis Rotation", "Fixed Open Rack")
datalot_levs = c("Location 1", "Location 2")
for (i in unique(energy_revenue$elcprc)) {
...
-   reap = Rural Energy for America Program reimburses 50% of capex (ttlcost) upfront. The waiting time for reimbursement is about 6 months. So, 50% of ttlcost acquire simple interest for six months.

-   inscst = insurance cost. \$5 per \$1000 capex.

-   taxcredit = 30% tax credit of annual cost covered through federal tax exemption.
...
```{r SSC2,echo=TRUE,eval=TRUE,error=TRUE,warning=TRUE}
...
```{r PlotAPFS,echo=TRUE,eval=TRUE,error=TRUE,warning=TRUE}
lox <- c("Northern", "Central", "Black Belt", "Southern")
array_levs = c("Single Axis Rotation", "Fixed Open Rack")
datalot_levs = c("Location 1", "Location 2")
...
```{r SQPrft,echo=TRUE,eval=FALSE,results='hide',error=TRUE,warning=TRUE}
...
```{r SQLong,echo=TRUE,eval=FALSE,results='hide',error=TRUE,warning=TRUE}
# Assign column names for clarity
...
```{r PrftSQuash,echo=TRUE,eval=FALSE,results='hide',error=TRUE,warning=TRUE}
...
```{r SPFFTBP,echo=TRUE,eval=FALSE,results='hide',error=TRUE,warning=TRUE}
ggplot(data = squash_long,
       mapping = aes(x = yield,
                     y = profit,
                     color = factor(price),
...
```{r SQAV,echo=TRUE,eval=FALSE,results='hide',error=TRUE,warning=TRUE}
...
```{r SQAVSave,echo=TRUE,eval=FALSE,results='hide',error=TRUE,warning=TRUE}
