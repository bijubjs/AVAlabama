...
```{r, echo=FALSE, include = FALSE, eval=FALSE, error=TRUE, warning=TRUE}
...
```{r, eval=FALSE,echo=FALSE,warning=TRUE,error=TRUE}
...
```{r,echo=FALSE,eval=FALSE,warning=TRUE,error=TRUE}
lox <- c("Northern", "Central", "Black Belt", "Southern")
array_levs = c("Single Axis Rotation", "Fixed Open Rack")
datalot_levs = c("Location 1", "Location 2")
...
energy_cost <- left_join(energy_cost, 
                         capex, 
                         by = c("array", "height")) %>% 
  # For Height = 7.75 ft. and Width = 3.5 ft., energy Output = 545 W.
  mutate(ttlcost = capex*545*panels,
...
```{r, echo=TRUE, eval=FALSE, cache=FALSE, error=TRUE, warning=TRUE}
...
# Apply the function to each combination of indices
...
# Same as above code but more efficient version. Checking the code.
...
```{r, echo=TRUE, eval=TRUE, cache=FALSE, error=TRUE, warning=TRUE}
# Efficient calculation of all combinations of rows from both matrices
solar_expanded <- solar_profit[rep(1:nrow(solar_profit), 
