...
Techno-economic analysis of agrivoltaic systems in Alabama. A paper for [Choice Magazine](https://www.aaea.org/publications/choices-magazine){.uri}, AAEA.

# Setting Up

## Housekeeping

```{r HK,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r WD,echo=FALSE,eval=TRUE,results='hide',error=TRUE,warning=TRUE}
path_mac = "/Users/bmishra/Library/CloudStorage/OneDrive-AuburnUniversity/Collaboration/Ngbede M/Choice-paper"
path_office = "C:\\Users\\bzm0094\\OneDrive - Auburn University\\Collaboration\\Ngbede M\\Choice-paper"
...
```{r Libs,,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r PacMan,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
pacman::p_loaded()
```

## Progress Bar

Tracking data processing progress. Code and results supressed.

```{r PB,echo=FALSE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
####### Progress Bar #####
pb = progress_bar$new(
...
```{r Theme,echo=FALSE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
####### Plotting Data: #####
# Map Theme:
plottheme <- ggplot() +
  theme_void() +
...
```{r ImportTomato,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r ImportSB,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r ImportSQ,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r ImpElcPrc,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r PVSC,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r CAPEXNREL,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r CpxPlt,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
capex %>%
  ggplot(aes(
    x = height,
    y = capex,
...
```{r PConf,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r EngOut, echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r EOBSPC,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r EOBDCSS,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
lox <- c("Northern", "Central", "Black Belt", "Southern")
ggplot(data = energy_output,
         mapping = aes(x = al_regs,
                       y = energy,
...
## Simulation 1 for energy revenue
...
```{r Sim1,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
## Simulation 2 for energy revenue
...
```{r Sim2,echo=FALSE,eval=FALSE,results='hide',error=TRUE,warning=TRUE}
## | results='hide'
# Sample data
set.seed(123)
...
```{r BDBNSPs,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
lox <- c("Northern", "Central", "Black Belt", "Southern")
array_levs = c("Single Axis Rotation", "Fixed Open Rack")
datalot_levs = c("Location 1", "Location 2")
...
```{r BDBPLUS, eval=FALSE, echo=TRUE, warning=TRUE, error=TRUE}
lox <- c("Northern", "Central", "Black Belt", "Southern")
array_levs = c("Single Axis Rotation", "Fixed Open Rack")
datalot_levs = c("Location 1", "Location 2")
for (i in unique(energy_revenue$elcprc)) {
...
-   opex = Operational cost (\$15/kW/Year); Source: [Ramasamy, 2022. PV Cost Benchmark](https://www.nrel.gov/docs/fy22osti/83586.pdf)
...
```{r SSC1,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r SSC2,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
r = 0.04 # Discount/interest Rate
n = 30 # Life Span of solar panels (Years)
...
energy_cost <- left_join(energy_cost, 
  ...
  mutate(
    ...
    anncost = ttlcost*(r*(1+r)**n)/((1+r)**n-1)+opex,
    moncost = ttlcost*((r/12)*(1+(r/n))^(n*12))/((1+(r/12))^(n*12)-1)+opex/12
    )
dim(energy_cost)
str(energy_cost)
head(energy_cost)
...
```{r PFS,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r PlotAPFS,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
lox <- c("Northern", "Central", "Black Belt", "Southern")
array_levs = c("Single Axis Rotation", "Fixed Open Rack")
datalot_levs = c("Location 1", "Location 2")
...
```{r PlotMPFS,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
lox <- c("Northern", "Central", "Black Belt", "Southern")
array_levs = c("Single Axis Rotation", "Fixed Open Rack")
datalot_levs = c("Location 1", "Location 2")
  for (i in unique(solar_profit$elcprc)) {
...
```{r PFTFilter,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r LDFTom,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r PFT,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r SBRet,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r sblong,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
# Assign column names for clarity
...
```{r PlotSBPBYv,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r PlotSBPBP,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
ggplot(data = stberry_long,
       mapping = aes(x = yield,
                     y = profit,
                     color = factor(price),
...
```{r SQPrft,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r SQLong,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
# Assign column names for clarity
...
```{r PrftSQuash,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r SPFFTBP,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
ggplot(data = squash_long,
       mapping = aes(x = yield,
                     y = profit,
                     color = factor(price),
...
```{r TAV,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r TAVSave,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r SBAV,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r SBAVSave,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r SQAV,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
...
```{r SQAVSave,echo=TRUE,eval=TRUE,results='markup',error=TRUE,warning=TRUE}
