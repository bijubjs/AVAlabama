...
Profit from solar alone at 100% PVD
...
write_xlsx(x = tav_profit %>%
    ...
    al_regs == "Southern" ~ 4,
    TRUE ~ NA_real_)),
           file = "Results/TAV Profit WriteUp R50.xlsx",
           as_table = TRUE)
```
...
write_xlsx(x = sbav_profit %>%
    ...
    al_regs == "Southern" ~ 4,
    TRUE ~ NA_real_)),
           file = "Results/SBAV Profit WriteUp R50.xlsx",
           as_table = TRUE)
```
...
sqav_profit <- sqav_profit %>%
  ...
  mutate(
    sqavp_ge_sq = if_else(yldvar == 1 & 
                            sqav_profit >= profit, 1, 0)
  ) %>%
  ungroup()
...
write_xlsx(x = sqav_profit %>%
    ...
    al_regs == "Southern" ~ 4,
    TRUE ~ NA_real_)),
           file = "Results/SQAV Profit WriteUp R50.xlsx",
           as_table = TRUE)
...
write_xlsx(
  ...
  file = "Results/SQAV Profit GE Squash R50.xlsx",
  as_table = TRUE
)
