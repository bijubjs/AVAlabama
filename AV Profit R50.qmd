...
\center \textbf NOTE: RUN "SUMULATION R50" BEFORE RUNNING THIS CODE FOR UPDATED INFORMATION.
...
tav_profit <- as.data.frame(
  read_feather(file = "Data/tav_profit R50.feather")
  )
dim(tav_profit)
```
...
write_xlsx(x = tav_be_yld,
           file = "Results/TAV Tomato Breakeven Yield R50.xlsx",
           as_table = TRUE)
```
...
```{r}
tcc_tav_tomato <- tav_profit %>% 
  filter(yldvar == 1,
         elcprc == 0.04,
         price == 20,
         sprop >= 0.1
         )
tcc_tav_tomato[which.max(tcc_tav_tomato$eannprofworeap),]
cat("Minimum REAP Compensation to make 
    TAV as profitable as Tomato: ",
    abs(max(tcc_tav_tomato$eannprofworeap)) + 9619.38, 
    fill = TRUE)

tcc_tav_tomato[which.min(tcc_tav_tomato$eannprofworeap),]
cat("Maximum REAP Compensation to make 
    TAV as profitable as Tomato: ",
    abs(min(tcc_tav_tomato$eannprofworeap)) + 9619.38,
    fill = TRUE)
```

## TAV = Tomato Scenarios

Which scenario produces same profit as or above tomato alone at 50% REAP?

```{r}
tav_equals_tomato_r50 <- tav_profit %>% 
  filter(yldvar == 0.5,
         elcprc == 0.04,
         price == 20,
         sprop >= 0.1,
...
sbav_profit <- as.data.frame(
  read_feather(file = "Data/sbav_profit R50.feather")
  )
dim(sbav_profit)
```

### SBAVP - Strawberry Profit
...
-   sbavp_wocp = sbav_profit - profit from 100% crop at their respective prices. This variable gives an idea where av profit stands in relation to crop profit. It helps to identify relative profitability of agrivoltaic system compared to crop only.
    ...
    # Step 3: sbavp_wocp = sqav_profit - unique profit
    ...
    unique_profits # 7 Prices give 7 Profits at 100% Yield.
    rm(unique_profits, profit_lookup)
    ```

    ### SBAVP GE Strawberry Profit
...
write_xlsx(x = sbav_be_yld,
           file = "Results/SBAV Breakeven Yield R50.xlsx",
           as.table = TRUE)
dim(sbav_be_yld); rm(results, split_data, process_subset)
```
...
```{r}
tcc_sbav_stberry <- sbav_profit %>%
  filter(yldvar == 1,
         elcprc == 0.04,
         price == 6,
         sprop >= 0.1
         )
tcc_sbav_stberry[which.max(tcc_sbav_stberry$eannprofworeap)]
cat("Minimum REAP Compensation to make SBAV 
    profitable as Strawberry: ",
    abs(max(tcc_sbav_stberry$eannprofworeap)) + 4175.66,
    fill = TRUE)

tcc_sbav_stberry[which.min(tcc_sbav_stberry$eannprofworeap)]
cat("Maximum REAP Compensation to make 
    SBAV profitable as Strawberry: ",
    abs(min(tcc_sbav_stberry$eannprofworeap)) + 4175.66,
    fill = TRUE)
```

## SBAV = Strawberry Scenarios

Which scenario produces same profit as or above tomato alone at 50% REAP?

```{r}
sbav_equals_sberry_r50 <- sbav_profit %>% 
  filter(yldvar == 0.5,
         elcprc == 0.04,
         price == 20,
         sprop >= 0.1,
...
# reversed column and row names (excluding elcprc in row_name)
...
write_xlsx(x = ct_tav_pft %>%
  ...
  dplyr::select(Row_Names, everything()),
           file = "Results/Profit Ctab TAV R50.xlsx",
           as_table = TRUE)
rm(result_matrix, sprop, array, height, elcprc, price);
rm(col_indices, col_names, i, missing_columns, row_data);
rm(row_condition, row_names, valid_indices, yldvar, al_regs);
rm(param_combinations, merged_data, required_columns);
dim(ct_tav_pft)
```
...
colorcount <- length(unique(
  as.vector(as.matrix(ct_tav_pft[-1]))))
...
rm(breaks, colorcount, custom_colors, min_val, neg_colors);
rm(pos_colors, zero_color, heatmap_plot, max_val)
```
...
ggsave(heatmap_plot,
       ...
       units = "in",
       limitsize = FALSE,
       file = paste0("Plots/TAV Profits CTab R50 Values", ".png"))
rm(breaks, colorcount, custom_colors, max_val, min_val, neg_colors)
rm(pos_colors, zero_color, heatmap_plot)
```

## TAV Profit Manuscript

This table is summarized in the manuscript.
...
write_xlsx(x = tav_prof_man %>%
  ...
  dplyr::select(Row_Names, everything()),
           file = "Results/TAV Profits Manuscript R50.xlsx",
           as_table = TRUE)
rm(result_matrix, sprop, array, height, elcprc, price, yldvar, al_regs)
rm(param_combinations, merged_data, row_data, col_names, i)
rm(missing_columns, required_columns, row_condition, row_names)
```

## TAVP - Tomato Profit CrossTab
...
-   See tav_profit for variable naming convention.
    ...
    rm(result_matrix, merged_data, param_combinations, row_data)
    rm(al_regs, array, col_names, elcprc, height, i, missing_columns)
    rm(price, required_columns, row_names, sprop, yldvar, row_condition)
    ```

```{r}
write.csv(as.data.frame(ct_tavp_wocp),
          row.names = TRUE,
          file = "Results/TAV Profit GE Tomato R50.csv")
```

## TAVP - Tomato Profit HeatMap
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/TAV Profit GE Tomato R50", ".png"))
rm(heatmap_plot); rm(colorcount)
```

## TAV Breakeven Yield Crosstab

```{r}
# Define the values for each variable
sprop <- c(0.05, 0.25, 0.50, 0.75, 
           0.80, 0.85, 0.90, 1)
array <- c("Fixed", "Tracking") # Solar Array
height <- c(4.6, 6.4, 8.2) # Panel height
al_regs <- c("Northern", "Central", "Black Belt", "Southern")
...
rm(result_matrix, merged_data, row_data, param_combinations); rm(al_regs, array, col_names, elcprc, height, i, missing_columns);rm(price, required_columns, row_condition, row_names, sprop, yldvar)
```

```{r}
write.csv(as.data.frame(ct_tav_be_yld),
          row.names = TRUE,
          file = "Results/TAV Breakeven Yield R50.csv")
```

## TAV Breakeven Yield HeatMap
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/TAV Breakeven Yield R50", ".png"))
...
combinations <- expand.grid(
  yldvar = c(0, 0.1, 0.3, 0.5, 0.7,
             1, 1.20, 1.5, 1.80, 2), # Yield
...
for (combo in seq_len(nrow(combinations))) {
  ...
  print(tav_sp_plot)
  ggsave(file = paste0("Plots/TAV by PVD R50", combo, ".png"))
  #break
}
...
for (combo in seq_len(nrow(combinations))) {
  ...
  print(combinations[combo,])
  print(tav_yv_plot)
  ggsave(file = paste0("Plots/TAV by Yield R50", combo, ".png"))
  #break
}
rm(combinations, combo)
```
...
write_xlsx(x = ct_sbav_pft %>%
  ...
  dplyr::select(Row_Names, everything()),
           file = "Results/SBAV Profit Ctab R50.xlsx",
           as_table = TRUE)
rm(result_matrix, sprop, array, height, elcprc, price, yldvar, al_regs); rm(col_indices, col_names, i, missing_columns, required_columns); rm(row_condition, row_names, valid_indices); rm(param_combinations, row_data, merged_data)
```
...
ggsave(heatmap_plot,
       ...
       units = "in",
       limitsize = FALSE,
       file = paste0("Plots/SBAV Profits Ctab R50 Values", ".png"))
rm(breaks, colorcount, custom_colors, max_val, min_val, neg_colors)
rm(pos_colors, zero_color, heatmap_plot)
```

## SBAV Profit Manuscript

This table is in manuscript.
...
write_xlsx(x = sbav_prof_man %>%
  ...
  dplyr::select(Row_Names, everything()),
           file = "Results/Profit SBAV Manuscript R50.xlsx",
           as_table = TRUE)
rm(result_matrix, sprop, array, height, elcprc, price, yldvar, al_regs)
rm(param_combinations, merged_data, row_data, col_names, i)
rm(missing_columns, required_columns, row_condition, row_names)
...
## SBAVP - Strawberry Profit CrossTab
...
write.csv(as.data.frame(ct_sbavp_wocp),
          row.names = TRUE,
          #col.names = TRUE,
          file = "Results/SBAV Breakeven Profits R50.csv")
dim(ct_sbavp_wocp)
```

## SBAVP - Strawberry Profit HeatMap
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/SBAV Breakeven Profits R50", ".png"))
rm(heatmap_plot, colorcount)
```

## SBAV Breakeven Yield Crosstab
...
write.csv(as.data.frame(ct_sbav_be_yld),
          row.names = TRUE,
          file = "Results/SBAV Breakeven Yield R50.csv")
```

## SBAV Breakeven Yield HeatMap
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/SBAV Breakeven Yield R50", ".png"))
...
for (combo in seq_len(nrow(combinations))) {
  ...
  print(combinations[combo,])
  print(sbav_sp_plot)
  ggsave(file = paste0("Plots/SBAV by PVD R50", combo, ".png"))
  #break
}
rm(combinations, combo)
```
...
for (combo in seq_len(nrow(combinations))) {
  ...
  print(combinations[combo,])
  print(sbav_yv_plot)
  ggsave(file = paste0("Plots/SBAV by Yield R50", combo,
                       ".png"))
}
rm(combinations, combo)
```
