...
```{r}
tcc_tav_tomato_r50 <- tav_profit %>% 
  filter(yldvar == 1,
         elcprc == 0.04,
         price == 20,
         sprop >= 0.1
         )
tcc_tav_tomato_r50[which.max(tcc_tav_tomato_r50$eannprofworeap),]
...
```{r}
tcc_sbav_stberry_r50 <- sbav_profit %>%
  filter(yldvar == 1,
         elcprc == 0.04,
         price == 6,
         sprop >= 0.1
         )
tcc_sbav_stberry_r50[which.max(tcc_sbav_stberry_r50$eannprofworeap)]
cat("Minimum REAP Compensation to make SBAV profitable as Strawberry: ",
    abs(max(tcc_sbav_stberry_r50$eannprofworeap)) + 1715.96, fill = TRUE)

tcc_sbav_stberry_r50[which.min(tcc_sbav_stberry_r50$eannprofworeap)]
cat("Maximum REAP Compensation to make SBAV profitable as Strawberry: ",
    abs(min(tcc_sbav_stberry_r50$eannprofworeap)) + 1715.96, fill = TRUE)
```
...
for (combo in seq_len(nrow(combinations))) {
  ...
  print(combinations[combo,])
  print(sbav_sp_plot)
  ggsave(file = paste0("Plots/sbav_sp_ R50", combo, ".png"))
  #break
}
```
...
for (combo in seq_len(nrow(combinations))) {
  ...
  print(combinations[combo,])
  print(sbav_yv_plot)
  ggsave(file = paste0("Plots/sbav_yv_ R50", combo, ".png"))
  #break
}
```
