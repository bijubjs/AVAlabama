...
cat("Minimum REAP Compensation to make TAV as profitable as Tomato: ",
    abs(max(tcc_tav_tomato_r25$eannprofworeap)) + 9619.38,
    fill = TRUE)
...
cat("Maximum REAP Compensation to make TAV as profitable as Tomato: ",
    abs(min(tcc_tav_tomato_r25$eannprofworeap)) + 9619.38,
    fill = TRUE)
```
...
-   sbavp_wocp = sbav_profit - profit from 100% crop at their respective prices. This variable gives an idea where av profit stands in relation to crop profit. It helps to identify relative profitability of agrivoltaic system compared to crop only.
    ...
    sbav_profit$sbavp_wocp <- mapply(function(sbav_profit,
                                              price) {
...
write_xlsx(x = ct_tav_pft %>%
  ...
  dplyr::select(Row_Names, everything()),
           file = "Results/TAV Profits Ctab R25.xlsx",
           as_table = TRUE)
dim(ct_tav_pft)
```
...
write_xlsx(x = tav_prof_man %>%
  ...
  dplyr::select(Row_Names, everything()),
           file = "Results/TAV Profits Manuscript R25.xlsx",
           as_table = TRUE)
# Display the result matrix
rm(result_matrix); rm(sprop); rm(array); rm(height); 
rm(elcprc); rm(price); rm(yldvar); rm(al_regs)
...
write.csv(as.data.frame(ct_tavp_wocp),
          row.names = TRUE,
          file = "Results/TAV Profits GE Tomato R25.csv")
```
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/TAV Profits GE Tomato R25", ".png"))
...
write.csv(as.data.frame(ct_tav_be_yld),
          row.names = TRUE,
          file = "Results/TAV Breakeven Yield R25.csv")
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/TAV Breakeven Yield R25", ".png"))
...
for (combo in seq_len(nrow(combinations))) {
  ...
  print(tav_sp_plot)
  ggsave(file = paste0("Plots/TAV by PVD R25", combo, ".png"))
  #break
}
```
...
for (combo in seq_len(nrow(combinations))) {
  ...
  print(combinations[combo,])
  print(tav_yv_plot)
  ggsave(file = paste0("Plots/TAV by Yield R25", combo, ".png"))
  #break
}
```
...
write_xlsx(x = ct_sbav_pft %>%
  ...
  dplyr::select(Row_Names, everything()),
           file = "Results/SBAV Profits Ctab R25.xlsx",
           as_table = TRUE)
dim(ct_sbav_pft)
```
...
write_xlsx(x = sbav_prof_man %>%
  ...
  dplyr::select(Row_Names, everything()),
           file = "Results/SBAV Profits Manuscript R25.xlsx",
           as_table = TRUE)
# Display the result matrix
rm(result_matrix); rm(sprop); rm(array); rm(height); 
rm(elcprc); rm(price); rm(yldvar); rm(al_regs)
...
write.csv(as.data.frame(ct_sbavp_wocp),
          row.names = TRUE,
          #col.names = TRUE,
          file = "Results/SBAV BE Sberry Profits R25.csv")
dim(ct_sbavp_wocp); rm(result_matrix)
```
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/SBAV BE Sberry Profit R25", ".png"))
...
write.csv(as.data.frame(ct_sbav_be_yld),
          row.names = TRUE,
          file = "Results/SBAV Breakeven Yield R25.csv")
dim(ct_sbav_be_yld)
```
...
ggsave(heatmap_plot,
       ...
       width = 12,
       units = "in",
       file = paste0("Plots/SBAV Breakeven Yield R25", ".png"))
...
```{r, results='hide'}
combinations <- expand.grid(
  yldvar = c(0, 0.1, 0.3, 0.5, 0.7, 1,
             1.20, 1.5, 1.80, 2), # Yield
...
for (combo in seq_len(nrow(combinations))) {
  ...
  print(combinations[combo,])
  print(sbav_sp_plot)
  ggsave(file = paste0("Plots/SBAV by PVD",
                       combo, ".png"))
}
```
...
for (combo in seq_len(nrow(combinations))) {
  ...
  print(combinations[combo,])
  print(sbav_yv_plot)
  ggsave(file = paste0("Plots/SBAV by Yield R25", 
                       combo, ".png"))
}
```
