...
```{r Housekeeping, warning=FALSE, error=TRUE}
# #| echo: TRUE
...
-   tavp_wocp = tav_profit - profit from 100% crop at their respective prices. This variable gives an idea where av profit stands in relation to crop profit. It helps to identify relative profitability of agrivoltaic system compared to crop only.
    ...
    # Step 3: tavp_wocp = tav_profit - unique profit
...
dim(tav_be_yld); rm(results, split_data, process_subset)
```
...
How much money should be spent as incentive to make AV as profitable as crop?
...
cat("Minimum Compensation to make SBAV profitable as Strawberry: ",
    abs(max(tcc_sbav_stberry$eannprofworeap)) + 4175.66, fill = TRUE)

tcc_sbav_stberry[which.min(tcc_sbav_stberry$eannprofworeap)]
cat("Maximum Compensation to make SBAV profitable as Strawberry: ",
    abs(min(tcc_sbav_stberry$eannprofworeap)) + 4175.66, fill = TRUE)
```
...
Tomato Yield at which TAV profit become breakeven with Tomato profit.
